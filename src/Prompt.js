 ...
 * @param {[string, string | boolean | number][]} options
 */
 ...
 * @prop {Array<string> | [string, string | number | boolean][]} arguments The arguments for this command
 */
...
class Prompt {
     ...
     */
    constructor({commands = [], prefix = {large: '--', sort: '-'}, options = {}} = {}) {
    ...
    reply(args = process.argv.slice(2)) {
        ...
        if (command.options?.length > 0) for (let argument of args) {
            let {options} = command;
            ...
            if (!optionData) {args.splice(args.indexOf(argument), 1);continue;}
                ...
                let checkedTypes = checkTypes(value, optionData.type);
    ...
    addCommands(...commands) {
        ...
        else this.commands.push(...commands);
